...
class SaveModelStrategy(fl.server.strategy.FedAvg):
    def __init__(self, model, data_type, *args, **kwargs):
        ...
        if not os.path.exists(f"checkpoints_predictor/{self.data_type}"):
            os.makedirs(f"checkpoints_predictor/{self.data_type}")
    ...
    def aggregate_fit(
        ...
        if aggregated_parameters is not None:
            ...
            # Save the model
            torch.save(self.model.state_dict(), f"checkpoints_predictor/{self.data_type}/model_round_{server_round}.pth")
...
def main() -> None:
    ...
    print(f"Saving metrics to as .json in histories_predictor folder...")
    # # check if folder exists and save metrics
    if not os.path.exists(f"histories_predictor/server_{args.data_type}"):
        os.makedirs(f"histories_predictor/server_{args.data_type}")
    with open(f'histories_predictor/server_{args.data_type}/metrics_{args.rounds}.json', 'w') as f:
